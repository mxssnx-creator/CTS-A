...
- Do **not** exclusive-lock 0.48/0.75. Use **all performing at or above live floors**. Live floors min TP **0.48** / min SL/TP **0.75**. TP 0.30–0.45 and SL/TP 0.50 are failed — intern may score them, live/settings must not.
- Block: additive **0.2**, shared **1.5**, overall **1.5**, maxMul **2.5**, scores N=1–6, **live volume 1,3,4,5,6 (skip N=2)**. Axis volume **3.0×** a normal position (live depth 2). **0.08 is forbidden** (Axis leftover that killed live).
