...
describe("complete 24h open tape", () => {
  it("busy-hour configs are the cells that paid", () => {
    ...
    assert.equal(ranges[0], "atr");
    assert.ok(ranges.includes("fibonacci"));
    assert.ok(ranges.includes("geometric"));
    assert.equal(expandPayRanges("ema", "fibonacci").length, 3);
    assert.equal(expandPayRanges("move", "atr").length, 3);
    ...
    assert.equal(legs.filter((l) => l.kind === "rng").length, 0);
    assert.ok(legs.some((l) => l.kind === "px" && l.range === "linear" && l.near < 0.11));
    ...
    assert.ok(ddLegs.some((l) => l.kind === "dd" && l.range === "volume" && l.near <= 0.12));
    assert.ok(ddLegs.some((l) => l.kind === "rng" && l.range === "geometric"));
    ...
    const lowLegs = indicationCalcLegs({ id: "ema", primary: "atr", dd: 0, pxStretch: 0.1, pays: true, busy: false, ddActivity: 0, relAlign: 0.4, bands });
    assert.ok(lowLegs.some((l) => l.kind === "rng" && l.range === "geometric"));
    const highLegs = indicationCalcLegs({ id: "ema", primary: "atr", dd: 0, pxStretch: 0.1, pays: true, busy: false, ddActivity: 0, relAlign: -0.4, bands });
    assert.ok(highLegs.some((l) => l.kind === "rng" && l.range === "volume"));
