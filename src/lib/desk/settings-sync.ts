  ...
  const snap: DeskSettingsSnap = {
    ...
    blockConfig: (() => {
      ...
      return {
        ...
        maxMultiple: Math.min(3, Math.max(1, Math.round(asNum(b.maxMultiple, d.blockConfig.maxMultiple)))),
        minMultiple: Math.min(2, Math.max(1, Math.round(asNum(b.minMultiple, d.blockConfig.minMultiple)))),
        ...
        counts: (() => {
          const rawCounts = Array.isArray(b.counts)
            ? [...new Set(b.counts.map((n) => Math.round(Number(n))).filter((n) => n >= 1 && n <= 3))].sort((a, c) => a - c)
            : [...(d.blockConfig.counts ?? [1, 2])];
          return rawCounts.length ? rawCounts.slice(0, 2) : [1, 2];
        })(),
